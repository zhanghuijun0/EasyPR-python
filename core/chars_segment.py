...
from core.core_func import getPlateType, Color, ThresholdOtsu, clearLiuDingChar
...
class CharsSegment(object):
    ...
    def verifyCharSizes(self, r):
        ...
        if (percPixels <= 1 and charAspect> minAspect and charAspect < maxAspect
            and r.shape[0] >= minH and r.shape[0] < maxH):
            return True
        else:
            return False
    ...
    def GetSpecificRect(self, vecRect):
        ...
        for i in range(len(vecRect)):
            mr = vecRect[i]
            midx = mr[0] + mr[2] / 2
            if (mr[2] > maxW * 0.8 or mr[3] > maxH * 0.8) and (midx < 2 * int(self.MatWidth / 7)
                                                             and midx > int(self.MatWidth / 7)):
                specIndex = i
        return  specIndex
